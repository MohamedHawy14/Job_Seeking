    ...
    <ul class="flex items-center space-x-6 mr-6 text-lg">
        <li x-data="{ open: false }" class="relative">
            <button @click="open = !open" class="hover:text-laravel flex items-center">
                <i class="fa-solid fa-globe"></i> {{ app()->getLocale() == 'ar' ? 'العربية' : 'English' }}
            </button>
            <div x-show="open" @click.away="open = false"
                class="absolute right-0 mt-2 py-2 w-32 bg-white border border-gray-200 rounded shadow-xl z-50">
...
<footer class="fixed bottom-0 left-0 w-full flex items-center justify-start font-bold bg-laravel text-white h-24 mt-24 opacity-90 md:justify-center">
    ...
    <a href="/listings/create"
        class="absolute top-1/3 end-10 bg-black text-white py-2 px-5">
        {{ __('main.post_job') }}
    </a>
</footer>
        <x-flash-message />
